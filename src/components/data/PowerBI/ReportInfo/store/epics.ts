    ...
    action$.pipe(
        ...
        switchMap((action) =>
            from(clients.report.getAccessDescription(action.payload)).pipe(
                ...
                catchError((error) =>
                    of(actions.fetchReportAccessDescription.failure({ action, error }))
                ),
                takeUntil(
                    action$.pipe(filter(isActionOf(actions.fetchReportAccessDescription.cancel)))
                )
            )
        )
    );
    ...
    action$.pipe(
        ...
        switchMap((action) =>
            from(clients.report.getDescription(action.payload)).pipe(
                ...
                catchError((error) =>
                    of(actions.fetchReportDescription.failure({ action, error }))
                ),
    ...
    action$.pipe(
        ...
        switchMap((action) =>
            from(clients.report.getRlsRequirements(action.payload)).pipe(
                ...
                catchError((error) =>
                    of(actions.fetchReportRequirements.failure({ action, error }))
                ),
...
export const initialize = (
    ...
    const request$ = (id: string) =>
        merge(
            ...
            fetchReportAccessDescription(
                of(actions.fetchReportAccessDescription.request(id)),
                state$,
                deps
            )
        );
    return action$.pipe(
        ...
        switchMap((action) =>
            concat(request$(action.payload), of(actions.initialize.success())).pipe(
                takeUntil(abort$)
            )
        )
    );
