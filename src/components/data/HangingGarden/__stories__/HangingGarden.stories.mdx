...
### provideController
  ...
  create yourself a ref like shown below. When it get populated you can trigger a rerender by calling clearGarden(). 
...
### Getting Started - Data source 
  First things first, you will need a data source. 
...
### Creating Columns 
  ...
  The component has no recognition of how the columns are layed out, or how you sorted them. 
...
#### highlightedColumnKey
  This is a good time to setup our highlightedColumn key if you want to use one. 
...
### Creating headers 
  ...
  This is the PIXI.Graphics object. It gives you a lo of freedom to create shapes and color in your header. 
  ...
  drawRoundedRect is a rectangle with rounded corner. You then add its coordinates inside the header as arguments. 
...
It has built in functionality for cache invalidation and refetch data. 
...
It will then either give you a errorMessage object back. That you can use with the standard error message boundary. 

You don't need to do check if there are error as the hook will handle this for you. 
And return a object with noError flag set. 
Else it will give back a proper response depending on Error. 
