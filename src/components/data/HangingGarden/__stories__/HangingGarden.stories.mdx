...
## Arguments implementations
  Implementation and use of each argument. What it expects to recive and what its used for. 
...
### columns
  ...
  You will need to write your own functionality, determening how your columns should be structured. 
...
### highlightedItem
  Takes entire object of an item you want to hightlight in the garden. 
  Item will receive be get a black outline when highlighted.
  The garden will also center the highlitedItem.
...
### itemWidth
  A number of pixels, represeting how wide your rendered items should be. 
...
### itemHeight
  The height of the rendered item. This follows the same prinsiples as itemWidth.
  All items in the garden will be same height. 
  But hight can be controlled in same fasion as itemWidth. 

### headerHeight
  The height of the header row at the top of the garden. 
  This follows the same prinsiples as itemWidth.
...
### onItemClick
  ...
  in the exmplane this is used to set the selected Item. 
  ```jsx
  (item: WorkOrderType) => setSelectedWorkOrder(item)
  ```
  This will then highlighted the item, and scroll to this item in the garden. 
  but this effect gives you alot of possibilities to trigger other effect or UI elemements migh want. 
  A typical use case is to open a sidesheet with extended information. 



### provideController
  Takes in a ref that will give you a way to rerender the garden outside the Garden Component. 
  In most cases the Garden will trigger its own rendering when needed. 
  But for some reason your functionality does not trigger a rerender when it should
  You can use this to trigger yourself. 
  create yoursel a ref like shown below. When it get populated you can trigger the rerendering by calling clearGarde(). 
  ...
  In most cases you should not need to handling this ourself. 
...
### renderItemContext
  ...
  The function will expand the context object by adding instruction for how items in the garden should be contructed. 
...
### renderHeaderContext
  ...
  Context will in similar fasion as renderItemContext be extended to dicated how the header looks. 
...
## Creating your own Hanging Garden 
  While the Hanging Garden component does alot of the heavy lifting in terms of how and when the garden renders. 
  Or how to interact with the underlying PIXI.js engine. 
  It still requires quite a few inputs to get it working properly. 

  This sections will guide you though how to make a similar garden as the one in the example on top. 
  While also talking about some more advanced features that is not used here. 

  Be aware that it will demand a decent time investment setting up your own Hanging Garden from scratch. 

### Getting Started - Data source 
  First things firts, you will need a data source. 
  The data can be what ever you need it to be, but you should be aware of its model.
  Hanging Garden component is generic and takes in the model of the data you will be using. 
...
### Creating Columns 
  Now that you have data and know what type of data you will be working with. 
  We can setup the columns structure. 

  This is all up to how its implemented. 
  The component has no recognision of how the columns are layed out, or how you sorted them. 
  ...
  It find our key by getting back the key name fron getYearAndWeek (example 2020/48). 
  Then decides if the column exists or not. If it exists we push the package into the data array. 
  Else we create a new object with this key and the item into the data propery. 

  When that is done we have our columns that are technicly ready to be sendt into the Garden. 
  but we do a couple more things to the array. 
  First we check if the column we have as highlitedColumn exists, if not we add it. 

  Then we sort the array, so early the year week is ascending. 

  The Garden will not add, remove, sort or manipualte data in anyway on its own. 
...
#### highlightedColumnKey
  This is a good time to setup our highlitedColumn key if you want to use one. 
  Our example uses the current week as a highlited column as shown above. 
  You want to make sure that this key string is also sendt in to the garden. 

### Creating headers 
  before creating your header, we should consider our header height. 
  ...
  context.isExpanded is supplid by the context. Telling us if this column is in the expanded state. 
  ...
  Highlited is dark green, so we want white text. 
  ...
  Making it centere both vertical and horizontal. 
  ...
  On the context you will find context.graphics. 
  This is the PIXI.Graphics object. It gives you alof of freedom to create shapes and color in your header. 
  ...
  context.graphics.drawRoundedRect(0,context.height - 5,context.widht,5,4);
  context.graphics.endFill();
  ```
  This will draw a green line though your header. 
  BeginFill to chose color, start the drawing. 
  drawRoundedRect is a rectangle with rounded corner. You then add its coordinatos inside the header as arguments. 
...
#### renderItemContext
  ...
  context has similiarites with the header context but is not the same. 
  ...
  ```
  createRect creates the rectangle for us. it takes in a position, size and backgroundcolor. 
  We here set the position to be top left. and use the full height and widht off our container.
  ...
  Next we need to set up the context.enquesRender. 
  ...
  In this function you will draw everything you need onto our item. 
  ```jsx
  ...
  We here set the workordernumber as text onto the item, then center it. 
...
These are used outside of the context.enquesRender function. 
...
You can add as multiple context.addDot to your renderItem function

##### context.addPopover 
This function is a hover effect for your item. 
...
```
We create a PIXI.Rectangle. The rectangle takes in its arguments that decides it placemet and size. 
...
### Some useful hooks 
The Garden component comes with some hooks that can be usefull. 
These are not implemented in the garden in anyway, but can be used if you find them helpful. 

#### useHangingGardenData 
This hooks fetched data based on the currentContext and the supplied endpoint.
What this hook gives you is cache handling.
It has built in functionality for cache invalidation and refetching data. 
It also give you the cache Age. 
...
You can take the error from useHangingGardenData and feed it into the hook. 
It will then either give you a errorMessage object back. That you can use with the standard error message boundry. 

You dont need to do check if there are error as the hook will handle this for you. 
And return a object with noError flag set. 
Else it will give back a proper response depening on Error. 
Outside of normal noData and general errors. 
It will handle noCache errors and access errors. 
