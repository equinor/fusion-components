...
export const getActivePositions = (positions: PersonPosition[], filterToDate: Date) =>
    positions.filter(
        (position) =>
            filterToDate.getTime() >= position.appliesFrom.getTime() &&
            filterToDate.getTime() <= position.appliesTo.getTime()
    );
export const getPastPositions = (positions: PersonPosition[], filterToDate: Date) =>
    positions.filter((position) => position.appliesTo.getTime() < filterToDate.getTime());

export const getFuturePositions = (positions: PersonPosition[], filterToDate: Date) =>
