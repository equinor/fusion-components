...
import ComponentDisplayContext, { componentDisplayTypes } from "../../contexts/ComponentDisplayContext";
...
const getButtonVariantClasses = props =>({
    [styles.contained]: props.contained || (!props.outlined && !props.frameless), // Default to contained
    [styles.outlined]: props.outlined,
    [styles.frameless]: props.frameless,
    [styles.icon]: Array.isArray(props.children) && props.children.find(c => typeof c  !== "undefined" && c.type === ButtonFontIcon),
});
...
const getButtonSizeClasses = (displayType, props) => ({
    [styles.small]: props.small || displayType === componentDisplayTypes.compact,
    [styles.medium]: (props.medium || (!props.small && !props.large)) && displayType !== componentDisplayTypes.compact, // Default to medium
    [styles.large]: props.large && displayType !== componentDisplayTypes.compact,
});

const getButtonClasses = (displayType, props) => classNames(
    props.className,
    styles.container,
    {
...
class Button extends Component {
    ...
    renderButton(displayType) {
        const { disabled, onClick, onClickCapture, url, targetBlank } = this.props;

        const containerClassNames = getButtonClasses(displayType, { ...this.props, ...this.state });
